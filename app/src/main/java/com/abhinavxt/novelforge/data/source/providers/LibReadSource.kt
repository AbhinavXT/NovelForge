...
open class LibReadSource : BrowseSource {
    ...
    override val filters = BrowseFilters(
        orderBys = listOf(
            FilterOption("Latest Release", "latest-release"),
            FilterOption("Latest Novels", "latest-novel"),
            FilterOption("Completed Novels", "completed-novel"),
        ),
        tags = listOf(
            ...
            "Sci-fi", "Seinen", "Shoujo", "Shounen Ai", "Shounen",
            "Slice of Life", "Smut", "Sports", "Supernatural", "Tragedy",
            "Wuxia", "Xianxia", "Xuanhuan", "Yaoi",
    ...
    override suspend fun browse(
        ...
        return try {
            // Genre pages and sort pages are separate URL trees (QuickNovel
            // scheme): a chosen genre overrides the sort list.
            val url = if (tag.isNullOrBlank())
                "$baseUrl/sort/${orderBy ?: "latest-release"}/$page"
            else
                "$baseUrl/genre/${tag.replace(" ", "+")}/$page"
            val document = fetchDocument(url) ?: return emptyList()
            parseListingPage(document)
        } catch (e: Exception) {
            Logger.e(name, "browse error (p$page)", e)
            emptyList()
        }
    }

    /** Parses one /sort/ or /genre/ listing page. Shared by browse/getPopular. */
