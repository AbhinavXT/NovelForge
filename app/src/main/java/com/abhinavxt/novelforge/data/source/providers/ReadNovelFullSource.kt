...
class ReadNovelFullSource : BrowseSource {
    ...
    // Genre listing URLs are unverified for this site, so only sort lists are
    // offered. (The /novel-list/<slug> pages below are the site's own nav.)
    override val filters = BrowseFilters(
        orderBys = listOf(
            FilterOption("Most Popular", "most-popular-novel"),
            FilterOption("Hot Novels", "hot-novel"),
            FilterOption("Completed", "completed-novel"),
            FilterOption("Latest Releases", "latest-release-novel"),
        ),
    )
    ...
    override suspend fun browse(
        ...
        return try {
            val url = "$baseUrl/novel-list/${orderBy ?: "most-popular-novel"}?page=$page"
            val document = fetchDocument(url) ?: return emptyList()
            parseSearchResults(document).distinctBy { it.id }
        } catch (e: Exception) {
            Logger.e("ReadNovelFull", "browse error (p$page)", e)
            emptyList()
        }
    }
