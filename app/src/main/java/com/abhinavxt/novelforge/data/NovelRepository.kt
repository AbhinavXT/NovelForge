...
import com.abhinavxt.novelforge.data.database.ReaderSettingsEntity
import com.abhinavxt.novelforge.data.database.CategoryEntity
import com.abhinavxt.novelforge.data.database.ChapterListItem
import com.abhinavxt.novelforge.data.database.HistoryRowData
import com.abhinavxt.novelforge.data.database.NovelCategoryCrossRef
import com.abhinavxt.novelforge.data.database.UpdateEntity
...
import com.abhinavxt.novelforge.data.model.PageTransition
...
class NovelRepository(
    ...
    private fun ReaderSettingsEntity.toDomainModel(): ReaderSettings {
        return ReaderSettings(
            ...
            tapZoneLayout = try {
                com.abhinavxt.novelforge.data.model.TapZoneLayout.valueOf(this.tapZoneLayout)
            } catch (e: Exception) {
                com.abhinavxt.novelforge.data.model.TapZoneLayout.SIDES
            },
        )
    }
     ...
     */
    suspend fun searchDownloadedContent(rawQuery: String): List<com.abhinavxt.novelforge.data.database.ChapterSearchResult> {
    ...
    suspend fun upsertCodexNames(names: List<com.abhinavxt.novelforge.data.database.CodexNameEntity>) =
        codexDao.upsertNames(names)

    suspend fun getCodexScanInfo(novelId: String) = codexDao.getScanInfo(novelId)

    suspend fun saveCodexScanInfo(info: com.abhinavxt.novelforge.data.database.CodexScanInfoEntity) =
        codexDao.saveScanInfo(info)

    suspend fun clearCodex(novelId: String) {
        codexDao.clearNames(novelId)
        codexDao.clearScanInfo(novelId)
    }

    /**
     * Chapters where [name] appears, spoiler-capped at
     * [maxChapterNumber]. Phrase-quoted so multi-word names match as
     * a unit; served by the FTS index, no codex-side mention table.
     */
    suspend fun getCodexMentions(
        novelId: String,
        name: String,
        maxChapterNumber: Int
    ): List<com.abhinavxt.novelforge.data.database.CodexMention> {
        val phrase = com.abhinavxt.novelforge.data.codex.CodexEngine.toPhraseQuery(name)
        return try {
            codexDao.getMentions(novelId, phrase, maxChapterNumber)
        } catch (e: Exception) {
            Logger.e("Codex mention lookup failed for \"$name\": ${e.message}")
            emptyList()
        }
    }

    /**
     * Chapter numbers where [name] appears (spoiler-capped) — the
     * relationship graph's raw material. No snippets, so ~25 of
     * these in parallel stay cheap.
     */
    suspend fun getCodexMentionChapterNumbers(
        novelId: String,
        name: String,
        maxChapterNumber: Int
    ): List<Int> {
        val phrase = com.abhinavxt.novelforge.data.codex.CodexEngine.toPhraseQuery(name)
        return try {
            codexDao.getMentionChapterNumbers(novelId, phrase, maxChapterNumber)
        } catch (e: Exception) {
            Logger.e("Codex chapter lookup failed for \"$name\": ${e.message}")
            emptyList()
        }
    }
