 ...
 */
class PawReadSource : BrowseSource {
    ...
    // URL scheme (QuickNovel): /list/{status}{genre}/{sort}/?page=N
    // e.g. /list/all-All/click/ = every status, every genre, by clicks.
    override val filters = BrowseFilters(
        categories = listOf(
            FilterOption("Completed", "wanjie-"),
            FilterOption("Ongoing", "lianzai-"),
            FilterOption("Hiatus", "hiatus-"),
        ),
        orderBys = listOf(
            FilterOption("Clicks", "click"),
            FilterOption("Time Updated", "update"),
            FilterOption("Time Posted", "post"),
        ),
        tags = listOf(
            "Fantasy", "Action", "Xuanhuan", "Romance", "Comedy", "Mystery",
            "Mature", "Harem", "Wuxia", "Xianxia", "Tragedy", "Historical",
            "Ecchi", "Adventure", "Adult", "Supernatural", "Psychological",
            "Drama",
        ).map { FilterOption(it, it) } + listOf(FilterOption("Sci-fi", "Scifi")),
    )
    ...
    override suspend fun browse(
        ...
        return try {
            val url = "$baseUrl/list/${category ?: "all-"}${tag ?: "All"}/" +
                "${orderBy ?: "click"}/?page=$page"
            val document = fetchDocument(url) ?: return emptyList()
            parseListingPage(document)
        } catch (e: Exception) {
            Logger.e("PawRead", "browse error (p$page)", e)
            emptyList()
        }
    }

    /** Parses one /list/ page. Same selector as search. */
    private fun parseListingPage(document: Document): List<NovelPreview> {
        val novels = mutableListOf<NovelPreview>()
        ...
        try {
            ...
            Logger.e("PawRead", "parseListingPage error", e)
        }
