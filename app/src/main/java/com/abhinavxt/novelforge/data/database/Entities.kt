...
/**
 * One auto-detected codex entry (character, place, faction) for a
 * novel. Populated by CodexEngine's heuristic scan of downloaded
 * chapter text; mention lookups are served live by the FTS index,
 * so this table only stores the aggregate.
 *
 * firstChapterNumber is the spoiler gate: the codex list hides
 * entries the reader hasn't reached yet.
 */
@Entity(
    tableName = "codex_names",
    primaryKeys = ["novelId", "name"],
    indices = [Index("novelId")]
)
data class CodexNameEntity(
    val novelId: String,
    val name: String,
    /** Total occurrences across all scanned chapters. */
    val occurrences: Int,
...
/**
 * Per-novel scan watermark so codex scans are incremental: only
 * chapters with number > lastScannedNumber are processed on rescan.
 */
@Entity(tableName = "codex_scan_info")
data class CodexScanInfoEntity(
    @PrimaryKey
    val novelId: String,
    val lastScannedNumber: Int,
